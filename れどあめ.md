...
このモードは、<details><summary>█████</summary>Kam7</details>によって**リークされた** Hypixel Lynx Mod との互換性を持たせる事ができるモードです。  
...
#### 説明
このコマンドを実行すると、プレイヤーの背後に、一定時間へばりつくNPCを召喚します。
...
#### 権限
特に指定がない場合は、```psr.mod```または、```psr.admin```です。


#### \<管理ID\>について
\<管理ID\>は、プレイヤーが報告を提出した際に、自動的に割当られる32文字の英数文字列です。  
このIDは、コンソールから ``` /psr view ``` をした際に表示されます。  
プレイヤーからも 管理ID に関連したコマンドを実行することができますが、**意味は全くもって無いでしょう。**

###### **注:** 管理IDに関するコマンドは```psac.mod```を持つプレイヤーも使用することはできますが、ヘルプに表示されません。
