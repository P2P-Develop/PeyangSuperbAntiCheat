...
詳しくは[こちら](PluginThreads.txt)（2015年10月21日16時29分現在）※2020年です←違います紀元前です←そんなわけないだろ

Firefoxと同様、メモリやCPUともにフードファイターの100倍くらい大食いします（ぺやんぐ調べ、書いた人はMozilla信者です）

リソースに余裕のあるサーバーで実行することをおすすめします。サーバーがぶっ壊れても知りません（壊れません、安心してください）どうせシングルスレッドだったらぶっ壊れるんだろ←Spigot自体シングルスレッドですが？なにか文句でも。

このプラグインでは、超原始的な本の実装方法を使用しています。そのため、サーバーが壊れるかもしれません（壊れるわけねぇよ）
...
- ちょー適当にクローンします。
- ぱぱっとMavenでビルドします。
- [ProtocolLib](https://www.spigotmc.org/resources/1997/)をなんとなーくpluginsフォルダーに入れます。
- このプラグインをさりげなくpluginsフォルダーに入れます。
- 気付かれないように再起動してください。
...
勝手にWatchdogが出勤してキックしたりだとか、110やら~~山田~~小林さんちの~~固定~~携帯電話やらに勝手に電話したりだとかそういった事は発生しないのでご安心を。
