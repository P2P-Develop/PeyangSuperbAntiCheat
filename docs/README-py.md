...
[**BungeeCordのつなぎ方を見たい人はこれみてくんね？**](BUNGEE.md)
...
「プログラムを書いてる」訳ではなく「開発」しております！（勘違いを誘発させる無駄な文章）

また、ハックと言う読み方は本来リバースエンジニアリングと言う行為を指す用語であり、クラッキングとは異なる可能性が発見されました。  
チートはプログラムを不正に改変し動作を変更するものであり、ハックと全く意味が違うような気がしたため、このREADMEでは整合性を保つためにチートとして扱っていました。

このプロジェクトは、[行動規範](CODE-OF-CONDUCT.md)の下でリリースされていることに注意してください。  
プロジェクトに参加することより…あなたは条件に従うことに同意するものとするかもしれないのデス！

ご理解のほど、よろしくお願いいたせくだ申し上げるのだ。 

↑ごめん＾＾

### 注意

**~~このプラグインはほぼネタプラグインｄ（ 殴（編集済）~~**

このプラグインは、常時2個DetectOPMで4個タイマーが動いているのだ。  
NPCが出てくるたびに、12個ほどタイマーが増えるのだ。  
すぐ消えますが、JavaのVMによって変わるのだ。  
詳しくは[こちら](PluginThreads.txt)をみてほしいのだ。（2015年10月21日16時29分現在）※2020年です←違います紀元前です←そんなわけないだろ←2021年だよもう

Firefoxと同様、メモリやCPUともにフードファイターの100倍くらい大食いするっぽいのだ（ぺやんぐ調べ、書いた人はMozilla信者です）

リソースに余裕のあるサーバーで実行することをおすすめします。サーバーがぶっ壊れても知りません（壊れません、安心してください）どうせシングルスレッドだったらぶっ壊れるんだろ←Spigot自体シングルスレッドですが？なにか文句でもあるのかなのだ？

このプラグインでは、超原始的な本の実装方法を使用しています。そのため、サーバーが壊れるかもしれません（壊れるわけないのだ）

---

### インストール方法

- [ここ](https://github.com/P2P-Develop/PeyangSuperbAntiCheat/releases)からダウンロードするのだ
- ちょー適当にクローンするのだ
- ぱぱっとMavenでビルドするのだ
- [ProtocolLib](https://www.spigotmc.org/resources/1997/)をなんとなーくpluginsフォルダーに入れるのだ。
- このプラグインをさりげなくpluginsフォルダーに入れるのだ。
- 気付かれないように再起動するのだ！！
...
勝手にWatchdogが出勤してキックしたりだとか、110やら~~山田~~小林さんちの~~固定~~携帯電話やらに勝手に電話したりだとかそういった事は発生しないのでご安心を。~~いつ買い物から帰ってくるんだ？~~

- スタッフ側

プレイヤーからのリポートは2形式で表示されるかもしれません。  
「\[PeyangSuperbAntiCheat\] プレイヤーがレポートを提出しました！クリックしてレポートを確認してください！ \[CLICK\]」  
または、  
「\[STAFF\] \[ADMIN\] Fishy: Report of \<PlayerName\> \<理由 1\>, \[理由 2\]...」  
の形で表示されます。コンフィグを編集することにより変更できますが、変更できる可能性があります。

このモードはKam7がリークしたHypixel Lynx（Mod）との互換性を持たせる事ができるモードなのだ。  
Lynxを導入したまま、Hypixelには**絶対**に参加したらやばいのだ。
Hypixelはスタッフ以外がLynxを持っていた場合、Lynxを持っているプレイヤーを自動的にBANするかもしれないのデス。
このModを作成したのは、すでに退職したAdminのRogue氏だと推測されまーすなのだ。
...
この本に表示されている報告事由をクリックすると、報告内容として事由が追加されるらしいですね。
...
**このコマンドの事由にはエイリアスを使用できます。下記の表を見てくださーい！**
...
指定されたプレイヤーに見えない矢を打ちます。 そのノックバックでVelocityかどうか判定してください。
...
`psac.mod`または、`psac.admin`権限を持っている人には以下のヘルプも表示されます。ハハッ
...
**削除ログは一切記録されません。破棄する場合には十分注意ださすぎワロタ**
...
このプラグインでは誤検出などのためにプレイヤーが誤BANされることを危惧してるらしい？のだ。  
そのため、プラグインが自動でプレイヤーをBANすることは**絶対に**あるかもしれませんがないと言い切っています ↑プラグインによってBANされたらIssueで気楽に教えてください。コラボレーターがPeyangに殴り込みにいくのだ。
...
スキンは同梱された（現時点では）1400種類のスキンセットからランダムで選ばれるだって？んな、いい話ねぇよ。

### コンフィグについて

このプラグインでは、以下のコンフィグを使用しています。適当に変えてください。 SQL関係は、[こ ↓ こ ↑](SQL.md)を参照してください。
...
心優しい英語ドキュメント担当者が機械学習を追加したのデス…愛に愛に愛に！実にいい！←これは本物の天才。Peyangとは違うね！  
キックの回数が増えるほど精度が~~下~~超上がります。  スメ　　　　　ルー
　　カレー　　　　　うどん　　　　　　　　　ポテイト
...
[コンフィグについて](#コンフィグについて)に記載されている通り、`lang`プロパティに指定することで簡単に言語の変更が。  
`lang`プロパティにはある程度曖昧な書き方ができるようにエイリアスも指定されてててててておりますお。  
エイリアスは以下のとおりなのだよ。
どうするかい？
...
イギリス英語も指定しているのは本プラグインの英語ではあまり違いがないと思われたたたたのだ
...
見つけたら報告お願い申し上げますだなも（？）  
↑金の亡者の声がするぜ…ﾊﾊｯ!ミキサーだよ

### [**こ ↑ こ ↓**](https://github.com/peyang-Celeron/PeyangSuperbAntiCheat/issues)場所の担当者

日本語：[ぺやんぐ](https://github.com/peyang-Celeron)←開発者←らしいっす。

日本語校正したり翻訳してる人：[Lemonade19x](https://github.com/Lemonade19x)←ぺやんぐの文章は「、」が多すぎたりサーバーをサーバなど言ったりしてるのでここのコミット200回以上やり直しましたなのだ！
...
- [Matrix API](https://matrix.rip/) [\(\*\)](https://www.mc-market.org/resources/13999/)
  ...
  <br>
  ↓ごめん、読めない。日本語でおｋだから。
解読した　あいうえおｴ?↓ﾅﾝﾀﾞｯﾃ!?0℃寒すぎﾋｬｯﾎｰ!!そりゃよ。寒すぎやッ!
