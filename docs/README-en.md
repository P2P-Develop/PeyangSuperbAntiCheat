...
***ANOTHER WARNING***: **With this plugin, detectopm will run +4 timers at the same time.  
...
This is an anticheat plugin for Bukkit / Spigot / PaperMC based server.  
...
```bash
/stop or /reload *stop is preferred*
```
or
```bash
$ java -jar server.jar
...
This mod is an instant-ban on Hypixel, so never use Lynx on Hypixel.  
[The Developer](https://github.com/peyang-Celeron) does not take any responsibility.**  
...
EN:
[PEYANG CHEAT DETECTION] A player has been removed from the game due to hacking or abuse.
```
...
EN:
Kicked the offending player from the game. Thank you for your report!
```
  
This message is sent when the watchdog automatically detects a cheat.  
For staff kicks, it will broadcast a different message.
...
A test message for this plugin. Please use it for testing / debugging
...
The NPC skin is displayed randomly by referring to the UUID skin sesetting.  
