...
Bukkit/Spigot/PaperMCのプラグインだった気がします。  
確信はございません。~~SUMANNA!~~
...
### 説明
このコマンドを実行するとプレイヤーの周りにプレイヤーの周囲を一定の速度で回るNPCを召喚します。  
このNPCに対して、一定の回数以上ヒットした場合、そのプレイヤーを自動でキックします。
...
### 説明
最強の浦沢をつかって、プレイヤーが導入しているForgeのModを一覧表示します。  
応答はModのIDで返答されます。脳内補完してください。  
Mod IDの変換は実装しま~~す~~ん。
...
2つ引数を指定した場合は、第1引数のプレイヤーが、第2引数のプレイヤーにテレポートします。
...
| コンパス | TRACKER | 対象にテレポートします。 | /stp <Target> |
