...
勝手にWatchdogが出動してキックしたりだとか110やら山田さんの固定電話やらに勝手に電話したりだとかそういった事は発生しないのでご安心を。しらんけど。
...
### こまんどおーばーらいどについて
PSACでは、コマンドを、オーバーライドする機能がこの度実装されました！やったね！  
~~不本意ですが、~~設定はプロパティで記述し、基本的にインデントを2スペースで入力します。  
```yaml
#こんふぃぐふぁいる
...
commands:
  kick:
    ...
    enable: true
```  
この例では、Kickコマンドを、PSACの物に置換する という動作が組まれています。  
PSAC製のKickコマンドでは、~~かっこいい最強の~~エフェクトをつけたり、Kickされるときに、でこれーしょんをつけられます。最強！  
