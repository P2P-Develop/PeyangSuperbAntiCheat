...
public class Books {
        ...
        public static ItemStack getReportBook(Player player, EnumCheatType... types) {
                ...
                for (EnumCheatType type : types) {
                        ...
                        component.append(text).event(new ClickEvent(ClickEvent.Action.RUN_COMMAND, "/report "
                                        + player.getName() + " " + tmpReasonText + " " + type.getSysName() + " \\"));
                }
                ...
                component.append(ChatColor.DARK_GREEN + ChatColor.BOLD.toString() + "レポートを提出")
                                .event(new ClickEvent(ClickEvent.Action.RUN_COMMAND,
                                                "/report " + player.getName() + " " + tmpReasonText + " bybooks"));

                component.append(ChatColor.RED + "レポートをキャンセル").event(new ClickEvent(ClickEvent.Action.RUN_COMMAND, ""));
        ...
        public static ItemStack getShowBook(String id, String uuid, String issueById, String issueByUuid,
                        BigDecimal dateInt, ArrayList<EnumCheatType> types) {
                ItemStack book = new ItemStack(Material.WRITTEN_BOOK);
                BookMeta meta = (BookMeta) book.getItemMeta();
                ...
                ComponentBuilder hover = new ComponentBuilder("UUID：").color(net.md_5.bungee.api.ChatColor.AQUA)
                                .append(uuid).color(net.md_5.bungee.api.ChatColor.GREEN);

                ComponentBuilder unixTime = new ComponentBuilder(TextBuilder.getLine("UNIX秒", String.valueOf(dateInt)));

                ComponentBuilder hover2 = new ComponentBuilder("UUID：").color(net.md_5.bungee.api.ChatColor.AQUA)
                                .append(issueByUuid).color(net.md_5.bungee.api.ChatColor.GREEN);
                ...
                b.append("\n");
                b.append(TextBuilder.getLine("対象者", id)).event(hoverEvt);
                b.append("\n");
                ComponentBuilder b1 = new ComponentBuilder(new TextComponent(b.create()));

                b1.append(TextBuilder.getLine("報告者", issueById)).event(hoverEvt2);
                b1.append("\n");

                ComponentBuilder b2 = new ComponentBuilder(new TextComponent(b1.create()));

                b2.append(ChatColor.DARK_RED + ChatColor.BOLD.toString() + "重大度："
                                + SeverityLevelUtils.getSeverity(types).getColor()
                                + SeverityLevelUtils.getSeverity(types).getText() + "\n");
                b2.append("\n");
